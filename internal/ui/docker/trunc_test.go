...
// wide runes may render wider than n columns, which is pre-existing
// table semantics and intentionally not covered here.
func TestTruncKeepsValidUTF8(t *testing.T) {
	...
	if got := trunc(emoji, 5); got != emoji {
		t.Fatalf("trunc bytes>n runes<=n = %q, want unchanged %q", got, emoji)
	}
}
