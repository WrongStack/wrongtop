...
- **Sensors** — CPU temperatures and fan RPMs on every platform it can get them: on macOS (arm64) WrongTop reads the AppleSMC directly via IOKit — real P-core temperatures that neither btop nor glances show on a Mac — plus battery state through IOPowerSources; Linux reads hwmon — fan speeds plus core and package temperatures (coretemp, k10temp, ACPI zones); Windows tries ACPI thermal zones and WMI
